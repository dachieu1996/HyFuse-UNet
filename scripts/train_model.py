...
DATASETS = {
    'opentextile': OpenTextileSegmentation,
    'spectralwaste': SpectralWasteSegmentation,
}


def infer_target_mode(dataset: str, input_mode: str | list[str]) -> str:
    if dataset == 'opentextile':
        return 'labels'

    if isinstance(input_mode, str):
        input_modes = input_mode.split(',')
    else:
        input_modes = input_mode

    if 'rgb' in input_modes:
        return 'labels_rgb'
    return 'labels_hyper_lt'
...
def main(args):
    args.target_mode = infer_target_mode(args.dataset, args.input_mode)
    args.experiment_name = f'{args.dataset}.{args.model}.{args.input_mode}.{args.target_mode}.{str(uuid.uuid4())[:4]}'
    ...
    train_data = dataset_cls(args.data_path, split='train', input_mode=args.input_mode, target_mode=args.target_mode, transforms=SemanticSegmentationTrain(), target_type='')
    val_data = dataset_cls(args.data_path, split='val', input_mode=args.input_mode, target_mode=args.target_mode, transforms=SemanticSegmentationTest(), target_type='')
    test_data = dataset_cls(args.data_path, split='test', input_mode=args.input_mode, target_mode=args.target_mode, transforms=SemanticSegmentationTest(), target_type='')
...
if __name__ == "__main__":
    ...
    # data paths
    parser.add_argument('--data-path', type=str, default='data/opentextile')
    parser.add_argument('--results-path', type=str, default='results')
    parser.add_argument('--device', type=str, default='cuda')
    # setting
    parser.add_argument('--dataset', type=str, choices=sorted(DATASETS.keys()), default='opentextile')
