...
from ..modules import FeatureRectifyModule as FRM
from ..modules import FeatureFusionModule as FFM
from ..modules import MSPABlock
...
class PredictorConv(nn.Module):
    ...
    def forward(self, x):
        B, C, H, W = x[0].shape
        x_ = [torch.zeros((B, 1, H, W), device=x[0].device, dtype=x[0].dtype) for _ in range(self.num_modals)]
...
class CMNeXt(nn.Module):
    def __init__(
        ...
        if len(in_channels) == 0:
            raise ValueError("CMNeXt requires at least one input modality")

        embed_dims, depths = cmnext_settings[model_name]
        if modals is None:
            modals = [f"modal_{idx}" for idx in range(len(in_channels))]
        if len(modals) != len(in_channels):
            raise ValueError("The number of modality names must match in_channels")
        ...
        # patch_embed
        self.patch_embed1 = PatchEmbed(in_channels[0], embed_dims[0], 7, 4, 7//2)
        ...
        if self.num_modals > 0:
            ...
            if any(c != extra_in_channels for c in in_channels[2:]):
                raise ValueError("CMNeXt requires all extra modalities to share the same number of channels")
            self.extra_downsample_layers = nn.ModuleList([
                PatchEmbedParallel(extra_in_channels, embed_dims[0], 7, 4, 7//2, self.num_modals),
        ...
        if self.num_modals > 0:
            self.extra_block1 = nn.ModuleList([MSPABlock(embed_dims[0], mlp_ratio=8, drop_path=dpr[cur+i]) for i in range(depths[0])])
        ...
        if self.num_modals > 0:
            self.extra_block2 = nn.ModuleList([MSPABlock(embed_dims[1], mlp_ratio=8, drop_path=dpr[cur+i]) for i in range(depths[1])])
        ...
        if self.num_modals > 0:
            self.extra_block3 = nn.ModuleList([MSPABlock(embed_dims[2], mlp_ratio=4, drop_path=dpr[cur+i]) for i in range(depths[2])])
        ...
        if self.num_modals > 0:
            self.extra_block4 = nn.ModuleList([MSPABlock(embed_dims[3], mlp_ratio=4, drop_path=dpr[cur+i]) for i in range(depths[3])])
