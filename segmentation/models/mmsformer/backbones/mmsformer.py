...
class MixTransformer(nn.Module):
    def __init__(self, model_name: str = 'B0', in_channels: int = 3, modality: str = 'depth'):
        super().__init__()
        ...
        # patch_embed
        self.patch_embed1 = PatchEmbed(in_channels, embed_dims[0], 7, 4, 7//2)
...
class MMSFormer(nn.Module):
    def __init__(
        ...
        if len(in_channels) == 0:
            raise ValueError("MMSFormer requires at least one input modality")
        ...
        if self.num_modals > 0:
            ...
            self.extra_mit = nn.ModuleList([
                MixTransformer(model_name, in_channels=in_channels[i + 1], modality=self.modals[i])
